...
To support this, a small amount of additional metadata would be needed: for each non-inline file, an offset mapping (`{ erofs_inode, layer_offset }`) so the client can locate objects within the (compressed) tar layer without downloading the whole thing. This would require the layers to use a seekable compression format (e.g. zstd:chunked, estargz, or future [registry-level compression](https://github.com/opencontainers/distribution-spec/issues/235)).
