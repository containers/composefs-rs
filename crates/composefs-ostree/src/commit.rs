...
impl<ObjectID: FsVerityHashValue> CommitWriter<ObjectID> {
    ...
    pub fn contains(&self, ostree_id: &Sha256Digest) -> bool {
        match self.lookup_idx(ostree_id) {
            Some(_) => true,
            None => false,
        }
    }
    ...
    pub fn serialize(
        ...
        // Add data refs
        for (i, e) in self.map.iter().enumerate() {
            let idx = if let Some(external_object) = &e.external_object {
                Some(ss.add_object_ref(&external_object))
            } else {
                None
            };
...
fn validate_buckets(buckets: &[u32; 256]) -> Result<()> {
    for i in 1..256 {
        ...
        if buckets[i] < buckets[i - 1] {
            return Err(Error::msg(format!("Invalid commit bucket data")));
        }
    }
    Ok(())
}
...
impl<ObjectID: FsVerityHashValue> CommitReader<ObjectID> {
    pub fn load(repo: &Repository<ObjectID>, content_id: &str) -> Result<Self> {
        ...
        let h = CommitHeader::ref_from_bytes(&buf)
            .map_err(|_e| Error::msg(format!("Invalid commit header")))?;
        ...
        let ostree_ids = Sha256DigestArray::ref_from_bytes(&buf)
            .map_err(|_e| Error::msg(format!("Invalid commit array")))?;
        ...
        for i in 0..item_count {
            ...
            map[i].data_offset = data.get_offset();
            map[i].data_size = data.get_size();
            map[i].external_object = if let Some(idx) = data.get_external_index() {
                ss.lookup_external_ref(idx as usize).cloned()
            } else {
                None
            };
        }
    ...
    fn get_data(&self, entry: &ReaderEntry<ObjectID>) -> &AlignedSlice<A8> {
        ...
        return &self.data[start..end].try_as_aligned().unwrap();
    }
    ...
    fn create_filesystem_dir(
        ...
        let (_obj_id, dirmeta) = self.lookup(dirmeta_id.into()).ok_or(Error::msg(format!(
            "Unexpectedly missing ostree dirmeta object {}",
            hex::encode(dirmeta_id)
        )))?;
        let (_obj_id, dirtree) = self.lookup(dirtree_id.into()).ok_or(Error::msg(format!(
