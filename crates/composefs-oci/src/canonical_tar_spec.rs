...
//! STATUS: DRAFT
//!
//! This document proposes a canonical, reproducible tar serialization for composefs filesystem trees. This complements the standardized EROFS metadata work by giving a deterministic mapping from filesystem tree to tar byte stream.
//!
//! ## Motivation
//!
//! Given a composefs filesystem tree (EROFS or dumpfile) and its content objects, it is often useful to regenerate the corresponding OCI tar layer: for example, to verify the OCI `diff_id`, or to reconstruct a layer that was not stored in its original tar form.
//!
...
//! ## Compression
//!
//! This specification defines the uncompressed tar byte stream only. Compression (gzip, zstd, or other framing) is a separate concern layered on top of the canonical byte stream without changing entry order or content.
//!
