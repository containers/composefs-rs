...
impl Storage {
    ...
    fn open_database(root_dir: &Dir) -> Result<Connection> {
        ...
        // Get the fd number and use /proc/self/fd path for SQLite.
        // SQLite's Connection::open opens its own fd to this path, so db_file
        // can be dropped normally after the connection is established.
        let fd_name = DecInt::from_fd(&db_file);
        let db_path = format!("/proc/self/fd/{}", fd_name.as_ref().to_string_lossy());
        let conn = Connection::open(&db_path)?;

        // db_file is dropped here, closing our fd. SQLite has its own fd.
        Ok(conn)
    }
